...
async function saveHealthReport() {
    ...
    .insert([
      {
        ...
        precautions,
        workouts,
        user_id: currentUser.id || currentUser?.id || currentUser?.user_metadata?.sub || currentUser?.sub
        // assuming you added RLS policy or this column
      }
    ]);
  ...
  showToast('Your reports are saved', 'success');
  savedReports.push([disease, description, medications, diets, precautions, workouts]);
  renderSavedReports();
}
...
async function deleteReport(reportId) {
  ...
  if (error) {
    ...
    // ✅ Remove from local savedReports array
    savedReports = savedReports.filter(report => report[6] !== reportId);
